...
namespace BasicESP
{
    public class Renderer : Overlay
    {
            ...
            if (enableESP)
            {
                foreach (var entity in entities)
                {
                    if (EntityOnScreen(entity))
                    {
                        DrawBox(entity);
                        DrawLine(entity);
                        DrawHealthBar(entity); // Добавляем отрисовку healthbar
                    }
                }
            }
        ...
        private void DrawHealthBar(Entity entity)
        {
            ...
            float healthPercentage = Math.Clamp((float)entity.health / 100.0f, 0.0f, 1.0f); // Предполагаем, что максимальное здоровье равно 100
            ...
            drawList.AddRectFilled(healthBarTop, healthBarBottom, ImGui.ColorConvertFloat4ToU32(new Vector4(0, 0, 0, 1))); // Черный фон
            drawList.AddRectFilled(new Vector2(healthBarTop.X, healthBarBottom.Y - filledHeight), new Vector2(healthBarBottom.X, healthBarBottom.Y), ImGui.ColorConvertFloat4ToU32(new Vector4(0, 1, 0, 1))); // Зеленый цвет заполнения
        }
